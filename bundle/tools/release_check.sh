...
case "$TYPE" in
  app|full)
    if [ -d bundle ]; then
      required=(wg-webui.sh VERSION README.md CHANGELOG.md manifest.json bundle/app/app.py bundle/app/requirements.txt bundle/app/core/__init__.py bundle/app/core/networks.py bundle/app/core/security.py bundle/app/templates/index.html bundle/app/static/css/app.css bundle/app/static/js/app.js bundle/VERSION bundle/PACKAGE_TYPE bundle/README.md bundle/CHANGELOG.md bundle/CONTRIBUTING.md bundle/SECURITY.md bundle/PROJECT_CONTEXT.md bundle/wg-webui.sh bundle/install.sh bundle/upgrade.sh bundle/uninstall.sh bundle/doctor.sh bundle/config/config.json.sample bundle/scripts/install.sh bundle/scripts/upgrade.sh bundle/scripts/uninstall.sh bundle/scripts/doctor.sh bundle/tools/repair_allowedips.py bundle/tools/sync_allowedips.py bundle/tools/repair_wg_nat.py bundle/tools/cleanup.py bundle/tools/release_check.sh bundle/tools/build_release.py bundle/tools/wireguard-userspace-compat-v0.5.0.tar.gz bundle/docs/INSTALL.md bundle/docs/CONFIGURATION.md bundle/docs/OPERATIONS.md bundle/docs/DEVELOPMENT.md bundle/docs/ROADMAP.md)
    else
      required=(app/app.py app/requirements.txt app/core/__init__.py app/core/networks.py app/core/security.py app/templates/index.html app/static/css/app.css app/static/js/app.js install.sh upgrade.sh uninstall.sh doctor.sh VERSION PACKAGE_TYPE README.md CHANGELOG.md CONTRIBUTING.md SECURITY.md PROJECT_CONTEXT.md config/config.json.sample scripts/install.sh scripts/upgrade.sh scripts/uninstall.sh scripts/doctor.sh tools/repair_allowedips.py tools/sync_allowedips.py tools/repair_wg_nat.py tools/cleanup.py tools/release_check.sh tools/build_release.py tools/wireguard-userspace-compat-v0.5.0.tar.gz docs/INSTALL.md docs/CONFIGURATION.md docs/OPERATIONS.md docs/DEVELOPMENT.md docs/ROADMAP.md)
    fi
    ;;
  deploy)
...
if [ -f CHANGELOG.md ]; then grep -q "## ${EXPECTED_TAG}" CHANGELOG.md || { echo "❌ CHANGELOG 缺少 ${EXPECTED_TAG} 条目"; exit 1; }; fi
